...
class OfflineStorageFallback {
  async storeData(item: Omit<OfflineDataItem, 'id' | 'timestamp' | 'encrypted' | 'synced'>): Promise<string> {
    console.warn('[OfflineStorage] storeData called in unsupported environment. Operation is a no-op.');
    return 'offline_fallback';
  }

  async retrieveData(_id: string): Promise<OfflineDataItem | null> {
    return null;
  }

  async getDataByType(_type: OfflineDataItem['type']): Promise<OfflineDataItem[]> {
    return [];
  }
  ...
  async clearDataByType(_type: OfflineDataItem['type']): Promise<void> {
    return;
  }

  async getStorageStats(): Promise<{ totalItems: number; syncedItems: number; unsyncedItems: number; storageSize: number; }> {
    return {
      totalItems: 0,
      syncedItems: 0,
      unsyncedItems: 0,
...
export const offlineStorage = hasIndexedDB
  ? new OfflineStorageManager()
  : new OfflineStorageFallback();
